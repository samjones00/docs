---
slug: templates-blazor-wasm
title: Blazor WebAssembly Template
---

We've created a template ideal for teams with strong C# skills that are looking to build a Line Of Business (LOB) application.
Utilizing Blazor WebAssembly (WASM) with ServiceStack as the backend, we get a development model that doesn't need additional tooling to manage shared request Data Transfer Objects (DTOs) between server and client.

With ServiceStack's [clean separation](/physical-project-structure) of ServiceStack's `ServiceModel`, combined with Blazor enabling C# on the client, we get complete reuse of your APIs shared DTOs as-is to enable an end-to-end Typed API automatically without additional tooling or code-gen.

## Shared ServiceModel Example

Usually with web applications, our client is using a different language from the server, so an equivalent request DTOs need to be generated for the client.
For example, TypeScript generated DTOs still give us typed end-to-end services with the help of tooling like [Add ServiceStack Reference](./add-servicestack-reference.md).
...
If the `Hello` request or response changes during development, the client DTOs need to be synchronized using commands like [`x csharp`](./add-servicestack-reference.md#simple-command-line-utilities).
...
And the client uses them directly in `.razor` pages.
...
It does this by compiling C# down to portable binary code format called [Web Assembly](https://webassembly.org). Web Assembly, or WASM is fast and runs on all major browsers.
...
As this defeats [many of the benefits](https://blazor-wasm.jamstacks.net/docs/hosting) of Blazor WASM in the first place we disregarded it & adopted
