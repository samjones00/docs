...
Not having to build projects also greatly simplifies deployments as it just becomes an exercise of deploying your Web App's static files, `web.settings` and the pre-compiled [Web App binary](https://github.com/NetCoreWebApps/WebApp). We're maintaining step-by-step guides for the most popular deployment methods in [templates.servicestack.net/docs/deploying-web-apps](http://templates.servicestack.net/docs/deploying-web-apps) which we'll continue adding to in future. 
...
[Admin Secret Key](http://docs.servicestack.net/debugging#authsecret):
...
Templates implementation of JavaScript preserves the Type which can be used to parse JavaScript or JSON literals, e.g:
