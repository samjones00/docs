...
We've [strongly recommended against usage of object and interface properties](https://stackoverflow.com/a/10759250/85785) on DTOS which is contra to the goals of designing self-describing APIs where they expose an unknown hole in Service contracts that's a potential source of runtime serialization issues, requires proprietary extensions to serialization formats and adds coupling to specific serializer implementations, embeds brittle .NET namespaces in DTOs and fails to serialize in many standards based serializers and as a result [won't work in some alternative languages](/swift-add-servicestack-reference#swift-native-types-limitations). 
...
To minimize the runtime Exceptions resulting from adopting a whitelist we've pre-populated it with a safe-list popular Data Types, DTOs and Request DTOs with the default configuration below:
