...
to add and update native Typed DTOs amongst its [8 supported languages](/add-servicestack-reference#supported-languages).
...
# Table of Contents
 ...
 - [New Vue and React "lite" ASP.NET Core Templates](#new-vue-and-react-lite-aspnet-core-templates)
    ...
    - [Integrated Bundling](#integrated-bundling)
        ...
        - [Register UMD Module Mappings](#register-umd-module-mappings)
...
![](https://raw.githubusercontent.com/ServiceStack/docs/master/docs/images/spa/create-react-app.png)
...
Browse [source code](https://github.com/NetCoreTemplates/vue-lite), view [vue-lite.web-templates.io](http://vue-lite.web-templates.io) live demo and install with:
...
Browse [source code](https://github.com/NetCoreTemplates/react-lite), view [react-lite.web-templates.io](http://react-lite.web-templates.io) live demo and install with:
...
We've also enabled a novel approach for updating your "lite" project assemblies where instead of everyone maintaining their own bespoke
configuration and a tool like **libman** for updating their local dependencies, **vue-lite** projects can just run:
...
We'll cover this in more detail when we cover our [`web` tool's](#web-tool) capabilities below.
...
routing to define your API at [/hello/_name/index.html](https://github.com/NetCoreWebApps/Blog/blob/master/hello/_name/index.html) (or `/hello/_name.html`)
...
`#Script` scripts are **run in the same context** and have access to the same functionality and features as a [Sharp App](https://sharpscript.net/docs/sharp-apps) 
including extensibility va [custom plugins](https://sharpscript.net/docs/sharp-apps#plugins). They can run **stand-alone** independent of an 
...
It builds on our last [v5.4 release](/releases/v5.4) where it served as a [Sharp App](https://sharpscript.net/docs/sharp-apps) 
...
.NET Core C# Templates:
   ...
   4. bare-webapp        .NET Core 2.1 Bare Templates WebApp
  ...
  10. parcel-webapp      .NET Core 2.1 Parcel Templates WebApp
  ...
  14. rockwind-webapp    .NET Core 2.1 Rockwind Templates WebApp
...
```
   1. init                 by @ServiceStack  Empty .NET Core 2.2 ServiceStack App                                      to: .                            [project]
   2. init-lts             by @ServiceStack  Empty .NET Core 2.1 LTS ServiceStack App                                  to: .                            [project]
   3. init-corefx          by @ServiceStack  Empty ASP.NET Core 2.1 LTS on .NET Framework                              to: .                            [project]
   4. init-sharp-app       by @ServiceStack  Empty ServiceStack Sharp App                                              to: .                            [project]
   5. bootstrap-sharp      by @ServiceStack  Bootstrap + Sharp Pages Starter Template                                  to: $HOST                        [ui,sharp]
   6. auth-memory          by @ServiceStack  AuthFeature configured with Memory Auth Repository and User Sessions      to: $HOST                        [auth]
   7. auth-sqlserver       by @ServiceStack  AuthFeature configured with SQL Server Auth Repository and User Sessions  to: $HOST                        [auth]
   8. auth-sqlite          by @ServiceStack  AuthFeature configured with SQLite Auth Repository and User Sessions      to: $HOST                        [auth]
   9. auth-postgres        by @ServiceStack  AuthFeature configured with PostreSQL Auth Repository and User Sessions   to: $HOST                        [auth]
  10. validation-contacts  by @ServiceStack  Contacts Validation Example                                               to: $HOST                        [example,sharp]
  11. vue-lite-lib         by @ServiceStack  Update vue-lite projects libraries                                        to: $HOST                        [lib,vue]
  12. react-lite-lib       by @ServiceStack  Update react-lite projects libraries                                      to: $HOST                        [lib,react]
  13. nginx                by @ServiceStack  Nginx reverse proxy config for .NET Core Apps                             to: /etc/nginx/sites-available/  [config]
  14. supervisor           by @ServiceStack  Supervisor config for managed execution of .NET Core Apps                 to: /etc/supervisor/conf.d/      [config]
  15. docker               by @ServiceStack  Dockerfile example for .NET Core Web Apps                                 to: .                            [config]

 Usage:  web +<name>
         web +<name> <UseName>

Search:  web + #<tag>      Available tags: auth, config, example, lib, project, react, sharp, ui, vue
```
...
Results matching tags [project,sharp]:
   ...
   4. init-sharp-app       by @ServiceStack  Empty ServiceStack Sharp App                  to: .      [project]
   5. bootstrap-sharp      by @ServiceStack  Bootstrap + Sharp Pages Starter Template      to: $HOST  [ui,sharp]
   6. validation-contacts  by @ServiceStack  Contacts Validation Example                   to: $HOST  [example,sharp]
...
From this list we can see that we can create an **Empty .NET Core 2.2 ServiceStack App** with:
...
To instead start with the **latest .NET Core LTS release**, run:
    ...
    $ web +init-lts ProjectName

After we've created our empty .NET Core project we can give it a **Bootstrap Sharp Pages UI** with:

    $ web +bootstrap-sharp ProjectName

What's even better is that gists can be chained, so we can create a **.NET Core 2.2 Bootstrap Sharp Pages App** with:

    $ web +init+bootstrap-sharp ProjectName

A **Bootstrap Sharp Pages App** that includes a complete **Contacts Validation example** with:

    $ web +init+bootstrap-sharp+validation-contacts ProjectName

The same as above, but its Auth replaced to persist in a **PostgreSQL** backend:

    $ web +init+bootstrap-sharp+validation-contacts+auth-postgres ProjectName

This isn't just limited to gist projects, you can also apply gists when **creating new projects**:

    $ web new sharp+auth-postgres
...
instead of copying .dll's in `/lib` folders. As a consequence you'll no longer have to build external ServiceStack GitHub projects or use GitHub published releases, 
...
This API is similar to `db.Dictionary<K,V>` where it uses the **first 2 columns** create a Dictionary using the first column for the key and
the 2nd column for the value but is more appropriate when you need to preserve ordering:
...
```hbs
{% raw %}{{ sql | dbSelect({ namedConnection }) }}
{{ sql | dbSelect({ connectionString:sqlServerConnString, provider:"mssql" }) }}{% endraw %}
```
...
# [v5.4 Release Notes](/releases/v5.4)
