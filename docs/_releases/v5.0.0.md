...
With `net45` builds always used when installed in any **.NET Framework** project. To allow **.NET v4.6.1+** Clients from sharing .NET Standard 2.0 assemblies with .NET Core Servers we continued to publish [.Core Client packages](#net-standard-client-packages-for-net-framework-projects) but we were quickly made aware there's also several Customers who are creating ASP.NET Core Apps running on the .NET Framework in order to be able to use existing .NET Framework assemblies at the sacrifice of being able to run on the faster and multi platform **.NET Core 2.0** runtime.
...
AutoQuery added support querying DISTINCT fields by prefixing the custom fields list with `DISTINCT `, e.g. `DISTINCT Field1,Field2`.
