...
internal caches and force existing pages to check if it has been modified, the next time it's called. 
...
By default it's only available to be called by **Admin** Users (or using an [AuthSecret](/debugging#authsecret)) but can
be changed with:

```csharp
Plugins.Add(new TemplatePagesFeature {
    TemplatesAdminRole = RoleNames.AllowAnyUser, // Allow any Authenticated User to call /templates/admin service
