...
[Razor Rockstars content pages](https://github.com/NetCoreTemplates/bare-webapp/blob/master/rockstars/dead/cobain/index.html), 
...
The difference with [bare-webapp](https://github.com/NetCoreTemplates/bare-webapp) is that the client source code is maintained in the `/client` folder and uses [Parcel JS](https://parceljs.org) to automatically bundle and publish your App to `/app/wwwroot` which is updated with live changes during development.
...
`JsonServiceClient.swift` source file you will need to [override the DefaultImports](https://docs.servicestack.net/swift-add-servicestack-reference#defaultimports):
...
[.NET HttpWebRequest Service Clients](https://docs.servicestack.net/csharp-client#httpwebrequest-service-clients).
...
[change which database connection](https://docs.servicestack.net/multitenancy#changedb-apphost-registration) your Services `Db` connection is configured with:
