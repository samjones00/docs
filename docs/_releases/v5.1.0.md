...
**[OrmLite](#ormlite)**
  ...
  - [Typed Column<T> and Table<T> APIs](#typed-columnt-and-tablet-apis)
...
[JsonServiceClient](https://pub.dartlang.org/documentation/servicestack/0.0.7/client/JsonServiceClient-class.html) available in the [servicestack](https://pub.dartlang.org/packages/servicestack#-installing-tab-) Dart package, to enable an end-to-end Typed API for calling Services by [sending and receiving rich native DTOs](/architecture-overview#client-architecture).
...
#### New TechStacks
  ...
  - URL: https://techstacks.io
  - Source Code: https://github.com/NetCoreApps/TechStacks

#### Old TechStacks

  - URL: http://angular.techstacks.io
  - Source Code: https://github.com/ServiceStackApps/TechStacks
...
The different domains means we need to transfer the Authenticated Session that's attached to https://techstacks.io by converting the Authenticated Session into a JWT Token that's sent to the App Server in parallel requests to retrieve the Authenticated User Info and convert the JWT Token into a Cookie so it's re-sent on every subsequent request to the App Server: 
...
### https://techstacks.io/servicestack
...
As a [reminder from last release](/releases/v5.0.0#enhancement-releases) we're dedicating time immediately after each release to resolve any issues. If any priority issues are reported and resolved we'll release them in new patch releases, e.g. **5.1.1**, **5.1.2**, etc. These will be non-breaking releases that will be safe to upgrade.
...
# [v5.0.2 Release Notes](/releases/releases/v5.0.0)
