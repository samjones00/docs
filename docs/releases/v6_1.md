...
The **Chinook** Locode App also showcases of how to implement an enhanced UX beyond Locode's default UI to create [Custom Forms](/locode/custom-forms.html) for its `Albums` table by creating custom HTML & JS Components which is loaded directly in Locode.
...
These attributes can be used to customize how fields and HTML Input controls in the Auto Form UIs in [Locode](https://locode.dev)
