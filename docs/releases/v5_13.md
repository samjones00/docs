...
Whilst HostingStartup is recommended going forward, its usage is optional and you can continue using ServiceStack's existing 
`ModularStartup` configuration if you're on an earlier .NET Core runtime. 
...
The `AppHost` registration has become slightly easier to enable ServiceStack in this release as you no longer need to manually inject 
...
First find the Project Template version you want by from the projects GitHub `/releases` page, e.g.
[/web/releases](https://github.com/NetCoreTemplates/web/releases) then clicking on the Release `<tag>`
to explore its contents. 

For the `web` template, [/v27](https://github.com/NetCoreTemplates/web/tree/v27) was the last release to target **net5.0** 
in its `.csproj`.

When you know which release you want, use the full URL of its Source Code **.zip** archive in place of the Template name, e.g:
...
[v27.zip](https://github.com/NetCoreTemplates/web/archive/refs/tags/v27.zip) archive and either manually rename the project 
and folder names or copy over the source code files you want into your existing solution.
