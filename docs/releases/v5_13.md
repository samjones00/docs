...
Whilst using `IHostingStartup` is recommended going forward, your existing `ModularStartup` classes will work without change when upgrading your projects to .NET6.

If you want to take advantage of the new .NET6 top-level `IHostBuilder` to side-step the need for a `Startup` class, you can still reuse and preserve your modular startup
