...
Whilst HostingStartup is recommended going forward, its usage optional and you can continue using ServiceStack's existing 
...
We're already forced to maintain multiple minimum .NET Framework version requirements in a number of packages with 3rd Party dependencies which 
...
the number of Customers affected to consider lowering our minimum .NET Framework requirements.
